...
mkdir -p "$HOME/.ssh"
KNOWN_HOSTS="${CC_RS_KNOWN_HOSTS:-$HOME/.ssh/known_hosts_cc_rs}"
SSH_OPTS=(
  -i cc-rs.pem
...
LOCK_DIR="${TMPDIR:-/tmp}/cc-rs-conn-${INSTANCE_ID}.lock"
if ! mkdir "$LOCK_DIR" 2>/dev/null; then
  echo "Another conn is running for ${INSTANCE_ID}."
...
if [[ "${SYNC}" != "0" ]]; then
  rsync -az -e "$RSYNC_SSH" --exclude target --exclude .git . ubuntu@"$PUBLIC_IP":~/cc-rs
else
