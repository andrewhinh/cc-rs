const RANGE_END: u32 = u32::MAX;

fn in_flat_ranges(range: &[u32], c: u32) -> bool {
    let mut i = 0;
    while i + 1 < range.len() {
        let lo = range[i];
        if lo == RANGE_END {
...
pub(crate) fn char_width(c: u32) -> u8 {
    const RANGE1: &[u32] = &[
        ...
        0xE0020, 0xE007F, 0xE0100, 0xE01EF, RANGE_END,
    ];

    if in_flat_ranges(RANGE1, c) {
        return 0;
    }
    ...
    const RANGE2: &[u32] = &[
        ...
        0x1F000, 0x1F644, 0x20000, 0x2FFFD, 0x30000, 0x3FFFD, RANGE_END,
    ];

    if in_flat_ranges(RANGE2, c) { 2 } else { 1 }
}

pub(crate) fn display_width_prefix(s: &str) -> usize {
    s.chars()
        .map(|ch| usize::from(char_width(u32::from(ch))))
        .sum()
}
